...
{
private:
	const std::map<std::string, int8_t> SENSOR_MODEL_TYPES =
	{ {"OS-0-32", 0}, {"OS-0-64", 1}, {"OS-0-128", 2},
	  {"OS-1-32", 3}, {"OS-1-64", 4}, {"OS-1-128", 5} };
	...
	uint8_t beam_altitude_angle_type_;
	const size_t NUM_PACKETS_PER_MESSAGE; // number of packets in each message to client (dds + callback)
	uint64_t msg_index_{ 0 };
...
public:
	SensorProxy(
		...
		const uint8_t beam_altitude_angle_type,
		...
		NUM_PACKETS_PER_MESSAGE(PACKETS_PER_FRAME * info.format.fps / notify_client_frequency ),
		beam_altitude_angle_type_(beam_altitude_angle_type),
